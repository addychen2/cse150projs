...
public class UserProcess {
    ...
    private List<UserProcess> children;
    private Map<Integer, Integer> childExitStatus;
    ...
    private int handleHalt() {
        // Only the root process (processID 0) can halt the machine
        if (processID != 0)
            return -1;
    ...
    private int handleExit(int status) {
        // Store exit status for parent to retrieve
        statusLock.acquire();
        exited = true;
        exitStatus = status;
        statusLock.release();
        ...
        UserKernel.numProcessesLock.acquire();
        UserKernel.numProcesses--;
        boolean isLastProcess = (UserKernel.numProcesses == 0);
        UserKernel.numProcessesLock.release();
    ...
    private int handleExec(int fileAddr, int argc, int argvAddr) {
        ...
        // Ensure the file ends with .coff (COFF executable)
        ...
        for (int i = 0; i < argc; i++) {
            // Read the i-th argument pointer
            int argPointer = argvAddr + i * 4;
            int bytesRead = readVirtualMemory(argPointer, buffer);
            if (bytesRead != 4) {
                return -1;
            }
            
            // Convert bytes to integer pointer
            int vaddr = Lib.bytesToInt(buffer, 0);
            
            // Read the string at the pointer
        ...
        // Create a new process and set parent/child relationship
    ...
    private int handleJoin(int processID, int statusAddr) {
        ...
        // Find the child with the specified process ID
        ...
        // Make sure it's actually a child of this process
        ...
        boolean hasExited = childProcess.exited;
        int exitStatus = childProcess.exitStatus;
        ...
        // Write the exit status to the statusAddr if provided
        if (statusAddr != 0) {
            byte[] buffer = new byte[4];
