...
public class KThread {
	...
	public static void yield() {
		...
		boolean intStatus = Machine.interrupt().disable(); //sets to false
	...
	/*
	One thread waits for another thread to finish before executing itself. 
	Multiple queues for a specific thread?
	Thread A would wait inside of join (queue) until thread B is finished. 
		-Each thread has it's own queue of children threads?
		-The thread runs until completion, so there is only 1 queue for the currently running thread. 
	*/
